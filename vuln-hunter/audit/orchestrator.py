    ...
    *,
    ...
    def _budget_check(stage_name: str) -> None:
        if max_tokens is not None:
            tokens = db.total_tokens(run_id)
            if tokens >= max_tokens:
                raise CostExceeded(
                    f"[{run_id}] token budget exhausted before {stage_name}: "
                    f"{tokens} >= {max_tokens} tokens "
                    f"(input={db.total_input_tokens(run_id)} "
                    f"output={db.total_output_tokens(run_id)})"
                )
        if max_cost_usd is not None:
            spent = db.total_cost(run_id)
            if spent >= max_cost_usd:
                raise CostExceeded(
                    f"[{run_id}] legacy dollar budget exhausted before {stage_name}: "
    ...
    try:
        ...
        log.info(
            "[%s] pipeline complete: total tokens %d — report at %s",
            run_id, db.total_tokens(run_id), report_path,
        )
        return report_path
        ...
        # Provider quota exhausted; run is resumable via --resume once
        # quota returns.
        log.error(
            "[%s] provider quota exhausted — aborting (resumable with --resume): %s",
            run_id, str(e)[:300],
        )
        db.finish_run(run_id, "aborted")
        raise
