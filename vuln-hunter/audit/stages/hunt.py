...
async def run_hunt(
    ...
    log.info(
        "[%s] hunt: dispatching %d tasks (concurrency=%d, model=%s, reasoning_effort=%s)",
        ctx.run_id,
        len(pending),
        sc.concurrency,
        sc.model,
        sc.reasoning_effort or "default",
    )
    ...
    async def _one(task: Task) -> None:
        async with sem:
            ...
            db.record_task_result(ctx.run_id, task.task_id, payload)
            findings = payload.get("findings", []) or []
            for f in findings:
                if db.add_finding(ctx.run_id, task.task_id, f):
                    counters["findings"] += 1
            ...
            log.info(
                "[%s] hunt %s: %d findings (tokens=%d in=%d out=%d)",
                ctx.run_id,
                task.task_id,
                len(findings),
                result.total_tokens,
