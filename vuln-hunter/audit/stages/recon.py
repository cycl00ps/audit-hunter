...
async def run_recon(ctx: StageContext, db: StateDB, max_tasks: int = DEFAULT_MAX_TASKS) -> dict:
    ...
    sc = ctx.stage("recon")
    log.info(
        "[%s] recon: model=%s reasoning_effort=%s max_tasks=%d",
        ctx.run_id,
        sc.model,
    ...
    for task in payload.get("initial_tasks", []):
        ...
        if db.add_task(ctx.run_id, task):
            added += 1
        else:
            skipped += 1

    log.info(
        "[%s] recon done: subsystems=%d entry_points=%d initial_tasks=%d "
        "queued=%d skipped_duplicates=%d tokens=%d in=%d out=%d",
        ctx.run_id,
